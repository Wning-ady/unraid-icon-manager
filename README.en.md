...
- Adds an independent wallpaper gallery for PNG/JPEG/WebP uploads, public-URL imports, downloads, deletion, host/container root copying, manual groups, and per-wallpaper classification. Hover over a wallpaper to use it as the UI background; the sidebar provides day/night buttons and a 0–30 px glass-blur slider. These preferences persist in `/config`.
...
Push a tag such as `v0.1.17` to publish these Docker Hub tags through GitHub Actions:
