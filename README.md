...
- 独立壁纸图库支持上传、URL 下载、手动分组与设为背景；左下角透明度滑块从 `0%`（完全显示壁纸、主色块透明）到 `100%`（主题纯色、壁纸不可见），设置随 `/config` 持久保存。
- 可选的虚拟机图标页通过 libvirt metadata 修改 `vmtemplate@icon`，支持 URL、上传和图库选择，不重启虚拟机。
...
下面是可以直接复制使用的**完整配置**，与仓库中的 [`docker-compose.yml`](docker-compose.yml) 保持一致。它完全不依赖 `.env`。保存为 `docker-compose.yml` 后，把三处 `192.168.1.10` 改成你的 Unraid IP：
...
| `image` | `waning/unraid-icon-manager:latest` | 要运行的镜像。`latest` 跟随最新版；希望升级可控时，直接改为 `waning/unraid-icon-manager:v0.1.18`。 |
...
| `volumes` | 八项挂载 | 前六项负责 Docker 图标，最后两项启用可选 VM 图标功能。每一项的具体权限见下表。 |
...
| `VM_ICONS_DIR` | `/unraid/vm-icons` | VM 用户是 | VM Manager 图标目录的容器路径，一般不要修改。 |
| `LIBVIRT_URI` | `qemu+unix:///system?socket=/var/run/libvirt/libvirt-sock` | VM 用户是 | 显式连接挂载的 Unraid libvirt socket，一般不要修改。 |
...
- Compose 安装如需固定版本，直接把 `image` 改为目标完整标签（例如 `waning/unraid-icon-manager:v0.1.18`），然后只更新本工具：
...
推送例如 `v0.1.18` 的标签后，GitHub Actions 会发布以下 Docker Hub 标签：

- `waning/unraid-icon-manager:latest`
- `waning/unraid-icon-manager:v0.1.18`
