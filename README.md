...
- Searches, multi-selects, saves reusable groups, and opens a focused single-container icon editor.
...
- Removes only the target container's Docker Manager RAM and persistent icon-cache files, then offers a **Refresh Docker Page** action using `UNRAID_DOCKER_URL`.
...
| `/var/lib/docker/unraid/images` | `/unraid/icon-cache` | Read/write | Invalidate only changed persistent Docker Manager icon files |
| `/usr/local/emhttp/state/plugins/dynamix.docker.manager/images` | `/unraid/icon-cache-ram` | Read/write | Invalidate only changed RAM icon files |
...
4. Use **Refresh Docker Page** after the targeted caches are invalidated. No application container is restarted.
...
- Keep the `/config` mount when upgrading; it contains uploaded icons, groups, audits, and backups.
...
Push a tag such as `v0.1.6` to publish these Docker Hub tags through GitHub Actions:
