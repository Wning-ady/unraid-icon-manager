...
export class AppDatabase {
  ...
  addAudit(record: Omit<AuditRecord, "id">): AuditRecord {
    const result = this.database.prepare(`INSERT INTO audits (container_name, template_file, old_icon, new_icon, backup_file, template_created, cache_backup, created_at, result) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(record.containerName, record.templateFile, record.oldIcon, record.newIcon, record.backupFile, record.templateCreated ? 1 : 0,
        record.cacheBackup ? JSON.stringify(record.cacheBackup) : null, record.createdAt, record.result);
  ...
  listAudits(limit = 100): AuditRecord[] {
    return this.database.prepare("SELECT * FROM audits ORDER BY id DESC LIMIT ?").all(limit).map((row: any) => ({
      ...
      cacheBackup: row.cache_backup ? JSON.parse(row.cache_backup) : null, createdAt: row.created_at, result: row.result
    }));
  }
