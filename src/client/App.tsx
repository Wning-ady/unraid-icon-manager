...
export function App() {
  ...
  return <main>
    ...
    <section><div className="section-title"><div><h2>当前 Docker 容器</h2><span>仅关联 Unraid 模板的容器可选择并写入图标</span></div><span className="result-count">{filtered.length} 个结果</span></div><div className="container-grid">{filtered.map((container) => <article className={`${selected.has(container.fileName ?? "") ? "card selected" : "card"}${container.editable ? "" : " readonly"}`} key={container.id} onClick={() => toggle(container)}><label className="card-select" onClick={(event) => event.stopPropagation()}><input aria-label={`选择 ${container.name}`} type="checkbox" disabled={!container.editable} checked={Boolean(container.fileName && selected.has(container.fileName))} onChange={() => toggle(container)} /></label><div className="icon">{container.icon?.startsWith("http") ? <img src={container.icon} alt="" /> : "▣"}</div><div className="card-content"><div className="card-topline"><strong>{container.name}</strong><span className={`state ${container.state}`}>{container.state}</span></div><p className="image-name">{container.image}</p>{container.editable ? <p className="template-note editable">可编辑 · 已关联 Unraid 模板</p> : <p className="template-note readonly-note">{container.uneditableReason === "compose" ? "Compose 容器 · 模板图标只读" : "没有匹配 Unraid 模板 · 无法持久化图标"}</p>}</div></article>)}</div></section>
