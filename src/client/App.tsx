...
interface Container {
  name: string; fileName: string; icon: string | null; image: string | null; state: string | null; status: string | null; managed: boolean;
}
...
export function App() {
  ...
  const refresh = async () => {
    try {
      ...
      setNotice(containerData.dockerAvailable ? "已读取 Unraid 模板与 Docker 状态。" : "已读取模板；Docker socket 不可用，运行状态未显示。");
  ...
  const toggle = (fileName: string) => setSelected((old) => {
    const next = new Set(old);
    if (next.has(fileName)) next.delete(fileName); else next.add(fileName);
    return next;
  });
  const selectAll = () => setSelected(new Set(filtered.map((container) => container.fileName)));
  ...
  async function saveGroup() {
    try { await request<Group>("/api/groups", { method: "POST", body: JSON.stringify({ name: groupName, containerNames: containers.filter((c) => selected.has(c.fileName)).map((c) => c.name) }) }); setGroupName(""); await refresh(); }
  ...
  return <main>
    ...
    <section><div className="section-title"><h2>容器</h2><span>仅显示存在 Unraid 模板 XML 的容器</span></div><div className="container-grid">{filtered.map((container) => <article className={selected.has(container.fileName) ? "card selected" : "card"} key={container.fileName} onClick={() => toggle(container.fileName)}><input type="checkbox" checked={selected.has(container.fileName)} onChange={() => toggle(container.fileName)} onClick={(event) => event.stopPropagation()} /><div className="icon">{container.icon?.startsWith("http") ? <img src={container.icon} alt="" /> : "▣"}</div><div><strong>{container.name}</strong><p>{container.image ?? "Docker 未连接"}</p><span className={`state ${container.state ?? "unknown"}`}>{container.state ?? "unknown"}</span></div></article>)}</div></section>
    <section className="split"><div><h2>分组</h2><div className="inline"><input placeholder="例如：媒体服务" value={groupName} onChange={(e) => setGroupName(e.target.value)} /><button disabled={!groupName || !selected.size} onClick={() => void saveGroup()}>保存当前选择</button></div>{groups.map((group) => <div className="row" key={group.id}><button className="link" onClick={() => setSelected(new Set(containers.filter((container) => group.containerNames.includes(container.name)).map((container) => container.fileName)))}>{group.name} · {group.containerNames.length}</button><button className="danger" onClick={() => void request(`/api/groups/${group.id}`, { method: "DELETE" }).then(refresh)}>删除</button></div>)}</div>
