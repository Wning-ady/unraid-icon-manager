...
function ModalIconGallery({ assets, groups, value, onSelect }: { assets: StoredIcon[]; groups: IconGroup[]; value: string; onSelect: (icon: string) => void }) {
  const [query, setQuery] = useState("");
  const [group, setGroup] = useState<number | "all" | "none">("all");
  const selectedFileName = iconFileName(value);
  const groupNames = new Map(groups.map((item) => [item.id, item.name]));
  const normalizedQuery = query.trim().toLowerCase();
  const filteredAssets = assets.filter((asset) => {
    const inGroup = group === "all" || (group === "none" ? asset.groupId === null : asset.groupId === group);
    const searchable = `${asset.fileName} ${asset.groupId === null ? "未分类" : groupNames.get(asset.groupId) ?? ""}`.toLowerCase();
    return inGroup && (!normalizedQuery || searchable.includes(normalizedQuery));
  });
  return <div className="modal-gallery"><div className="modal-gallery-tools"><input aria-label="搜索图库图标" value={query} placeholder="搜索文件名或分组…" onChange={(event) => setQuery(event.target.value)} /><select aria-label="筛选图库分组" value={group} onChange={(event) => setGroup(event.target.value === "all" || event.target.value === "none" ? event.target.value : Number(event.target.value))}><option value="all">全部分组 ({assets.length})</option><option value="none">未分类 ({assets.filter((asset) => asset.groupId === null).length})</option>{groups.map((item) => <option key={item.id} value={item.id}>{item.name} ({assets.filter((asset) => asset.groupId === item.id).length})</option>)}</select></div>{filteredAssets.length ? filteredAssets.map((asset) => { const chosen = selectedFileName === asset.fileName; const groupName = asset.groupId === null ? "未分类" : groupNames.get(asset.groupId) ?? "未分类"; return <button type="button" key={asset.fileName} className={chosen ? "chosen" : ""} aria-label={`选择 ${groupName} 中的图标`} aria-pressed={chosen} title={`${groupName} · ${asset.fileName}`} onClick={() => onSelect(asset.icon)}><img src={asset.previewUrl} alt="" /></button>; }) : <span>{assets.length ? "当前搜索或分组没有匹配图标。" : "图库为空，请先上传一个图标。"}</span>}<small className="modal-gallery-count">显示 {filteredAssets.length} / {assets.length}</small></div>;
}
...
export function App() {
  ...
  const [page, setPage] = useState<"dashboard" | "vms" | "gallery" | "wallpapers" | "about">("dashboard");
  ...
  const refresh = async (message?: string) => {
    try {
      const [containerData, vmData, auditData, galleryData, iconGroupData, aboutData, wallpaperData, groupData, uiSettings] = await Promise.all([
        request<{ containers: Container[]; dockerAvailable: boolean }>("/api/containers"), request<{ vms: VirtualMachine[]; libvirtAvailable: boolean }>("/api/vms"),
        request<Audit[]>("/api/audits"),
        request<StoredIcon[]>("/api/icons"), request<IconGroup[]>("/api/icon-groups"), request<AboutMeta>("/api/about"), request<StoredWallpaper[]>("/api/wallpapers"), request<WallpaperGroup[]>("/api/wallpaper-groups"), request<UiSettings>("/api/ui-settings")
      ]);
      setContainers(containerData.containers); setAudits(auditData);
      setVms(vmData.vms); setLibvirtAvailable(vmData.libvirtAvailable);
      setGallery(galleryData); setIconGroups(iconGroupData); setAbout(aboutData);
      setWallpapers(wallpaperData); setWallpaperGroups(groupData);
      setTheme(uiSettings.theme); setActiveWallpaperFileName(uiSettings.wallpaperFileName); setGlassBlur(uiSettings.glassBlur); setSurfaceOpacity(uiSettings.surfaceOpacity);
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty("--surface-opacity", String(surfaceOpacity / 100));
    document.documentElement.style.setProperty("--glass-surface-opacity", String((surfaceOpacity / 100) * 0.12));
  ...
  const filteredIcons = gallery.filter((asset) => selectedIconGroup === "all" || (selectedIconGroup === "none" ? asset.groupId === null : asset.groupId === selectedIconGroup));
  const iconTargetName = typeof selectedIconGroup === "number" ? iconGroups.find((group) => group.id === selectedIconGroup)?.name ?? "未分类" : "未分类";
  ...
  async function applyVmIcon() {
    if (!editingVm || !vmIcon.trim()) return;
    setBusy(true); setVmError("");
    try {
      const result = await request<{ notice: string; refreshUrl: string }>(`/api/vms/${editingVm.id}/icon`, { method: "POST", body: JSON.stringify({ icon: vmIcon }) });
      setEditingVm(null); setShowVmGallery(false); await refresh(result.notice);
      window.open(result.refreshUrl, "_blank", "noopener,noreferrer");
    } catch (error) { setVmError(`保存失败：${error instanceof Error ? error.message : "未知错误"}`); }
    finally { setBusy(false); }
  }
  ...
  async function createIconGroup() {
    ...
    setBusy(true);
    try { await request("/api/icon-groups", { method: "POST", body: JSON.stringify({ name: newIconGroupName }) }); setNewIconGroupName(""); await refresh("图标分组已创建。"); }
    catch (error) { setNotice(`创建分组失败：${error instanceof Error ? error.message : "未知错误"}`); }
    finally { setBusy(false); }
  }

  async function moveIcon(asset: StoredIcon, groupId: number | null) {
    try { await request(`/api/icons/${asset.fileName}`, { method: "PATCH", body: JSON.stringify({ groupId }) }); await refresh("图标分类已更新。"); }
  ...
  async function updateUiSettings(patch: Partial<UiSettings>, message: string) {
    try {
      ...
      setTheme(settings.theme); setActiveWallpaperFileName(settings.wallpaperFileName); setGlassBlur(settings.glassBlur); setSurfaceOpacity(settings.surfaceOpacity); setNotice(message);
